...
namespace ML
{
    public class Usuario
    {
        ...
        [RegularExpression("/^[A-Za-z0-9\\s]+$/g", ErrorMessage ="El nombre de usuario debe tener al menos un numero y letras")]
        ...
        [RegularExpression("/^\\w+([.-_+]?\\w+)*@@\\w+([.-]?\\w+)*(\\.\\w{2,10})+$/", ErrorMessage = "El email no es valido")]
        public string Email { get; set; }
        [DisplayName("Password")]
        [Required(ErrorMessage = "Escribe el password")]
        [RegularExpression("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[$@@$!%*?&])([A-Za-z\\d$@@$!%*?&]|[^ ]){8,15}$/", ErrorMessage = "El password debe contener al menos un número, una mayuscula y un signo")]
        ...
        [RegularExpression("/^\\d{10}$/", ErrorMessage = "El telefono solo debe incluir números")]
        public string Telefono { get; set; }
        [DisplayName("Celular")]
        [Required(ErrorMessage = "Escribe el celular")]
        [RegularExpression("/^\\d{10}$/", ErrorMessage = "El celular solo debe incluir números")]
